...
```text
Define and enforce a production OCI sandbox profile for every untrusted workload. In addition to no_new_privileges and an empty capability set, run as a non-root user by default where the image permits it, use a read-only root filesystem with explicit writable tmpfs and volume mounts, set PID and process limits, apply a maintained seccomp profile, enforce an AppArmor or SELinux profile when supported, expose no host devices or sockets, isolate IPC/UTS/network/mount namespaces, mask sensitive proc/sys paths, and prohibit privileged, host-network, host-PID, arbitrary sysctl, device, and bind-mount configuration through the customer API. Enforce hard cgroup v2 CPU, memory, swap, and OOM behavior and reserve resources for the agent/runtime so tenant pressure cannot kill reconciliation. Make any compatibility relaxation an operator-defined named profile with visible risk and audit history, not arbitrary OCI input. Add adversarial VM tests for host filesystem/socket access, process exhaustion, memory pressure, namespace escape prerequisites, device access, and noisy-neighbor containment.
```
