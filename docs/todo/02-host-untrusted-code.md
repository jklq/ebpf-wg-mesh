...
Implemented: every successful build records an immutable `build_artifacts` row (source snapshot digest, commit SHA, build recipe, builder toolchain version, image repository and manifest digest, build actor, timestamps), and that artifact — not an image string — is the runtime identity of deployments, deployment transitions, and service rollouts. Direct-image deploys resolve a tag to a manifest digest at deploy time through the OCI Distribution API (anonymous pull; credentialed registries fail closed with an actionable error) and store the same artifact shape with the mutable tag retained only as `source_image_ref` user input. Resolution is fenced against hostile registries: the registry host must be a public destination unless the operator allowlists it (`CONTROLPLANE_DIRECT_IMAGE_ALLOWED_PRIVATE_REGISTRIES`), Bearer token realms stay on the registry's own site, and manifest redirects are refused. Source build requests are fenced against the binding's proven head commit — pushes must chain from it via the payload's `before`, tracked-head syncs just fetched it — so a delayed webhook for an unseen older commit is recorded as history but can never supersede queued newer work or regress a rollout. When the same source snapshot and recipe already produced an image for the service, the build is skipped and the existing artifact rolls out with the service's current variables (deployment reason `BUILD_REUSED`), including retries and webhook redeliveries. Retention keeps every artifact a deployment, transition, rollout, or the current pointer references — the exact material rollback and exact redeploy need — no matter its age, and prunes only unreferenced artifacts older than the retention window beyond the newest keep-recent per service (keep-recent ranks among the aged-out candidates; unreferenced artifacts inside the retention window are never pruned). Tests cover tag mutation after resolve (the stored digest still runs), skip-rebuild with current variables, artifact record contents, and retention of rollback material.

Remaining: console surfaces for artifact provenance and digest-pinned identity; see `docs/frontend-handoff/2.6.md`.
