...
```text
Add an optional pre-deploy command that runs from the newly built image as a bounded one-shot allocation with the service’s target configuration and explicitly selected secrets, but without receiving public ingress or volume mounts. A successful job permits rollout; failure or timeout leaves the active generation untouched and records logs and exit cause. Ensure retries are explicit because database migrations may not be idempotent. Persist progress so a control-plane restart resumes safely. Do not create a general job/workflow engine or an environment-wide deploy DAG. Test migrations, cancellation, timeout, and unchanged services.
```
