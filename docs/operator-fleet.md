...
| `draining` | no | Stateless allocations are replaced through the rolling reconciler: a new allocation is created on another active node, ingress switches, then the old process drains. Volume-backed allocations stay on the draining node. Existing allocation identities are never rewritten onto another node. |
