...
func (s *persistence) beginAgentSessionTx(ctx context.Context, tx *sql.Tx, agentID, sessionID string, now time.Time) error {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE agent_administration
		...
		WHERE agent_id = $1 AND lifecycle_state <> 'retired' AND credential_revoked_at IS NULL`, agentID, now); err != nil {
		return err
	}
	result, err := tx.ExecContext(ctx, `UPSERT INTO agent_presence(
		agent_id, session_id, last_observation_sequence, last_contact_at, ready, reachable, updated_at
	) VALUES ($1, $2, 0, $3, TRUE, TRUE, $3)`, agentID, sessionID, now)
	if err != nil {
		return err
	}
...
func (s *persistence) setAgentAdministrationTx(ctx context.Context, tx *sql.Tx, agentID string, state AgentLifecycleState, intent, message string, revokedAt sql.NullTime, now time.Time) error {
	_, err := tx.ExecContext(ctx, `UPDATE agent_administration
		...
		WHERE agent_id = $1`, agentID, state, intent, message, revokedAt, now)
	return err
}
...
func (s *persistence) setAllocationStateTx(ctx context.Context, tx *sql.Tx, allocationID, rolloutState, intent, message string, drainStarted, drainDeadline sql.NullTime, now time.Time) error {
	_, err := tx.ExecContext(ctx, `UPDATE allocation_assignments
		...
		WHERE id = $1`, allocationID, rolloutState, intent, message, drainStarted, drainDeadline, now)
	return err
}
