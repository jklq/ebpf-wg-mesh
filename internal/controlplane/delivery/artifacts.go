...
// insertBuildArtifactTx records an immutable artifact, converging
// concurrent writers on one row per (service, image). Callers must pass a
// digest-pinned image: user tags are resolved before the product
// transaction and never reach this insert.
func (s *persistence) insertBuildArtifactTx(ctx context.Context, tx *sql.Tx, params insertArtifactParams) (BuildArtifactRecord, error) {
	...
	recipeJSON, err := source.MarshalBuildRecipe(params.BuildRecipe)
	if err != nil {
		return BuildArtifactRecord{}, err
	}
	reuseKey := ""
	...
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO build_artifacts(
			id, service_id, build_id, kind, source_snapshot_digest, commit_sha,
		...
		id, params.ServiceID, params.BuildID, params.Kind, params.SourceSnapshotDigest, params.CommitSHA,
		recipeJSON, params.BuilderVersion, params.ImageRepository, params.ImageManifestDigest,
		imageRef, params.SourceImageRef, reuseKey, params.BuildActorKind, params.BuildActorID, params.CreatedAt,
	); err != nil {
		return BuildArtifactRecord{}, err
	}
	return scanBuildArtifactRow(tx.QueryRowContext(ctx,
		`SELECT `+buildArtifactSelectColumns+`
		  ...
		  WHERE service_id = $1 AND image_ref = $2`,
		params.ServiceID, imageRef,
	))
}
