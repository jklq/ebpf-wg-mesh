...
func (s *Store) replaceLostNodeAllocationTx(ctx context.Context, tx *sql.Tx, deadAgentID string, allocation allocationRecord, now time.Time) (nodeLossReplacementResult, error) {
	...
	blockedMessage := ""
	switch {
	case project.Kind == projectKindManaged:
	...
	if blockedMessage != "" {
		changed, err := markAllocationUnavailableForFailover(ctx, tx, allocation.ID, state, blockedMessage, now)
		if err != nil {
			return result, err
		}
		result.Blocked = true
	...
	if _, err := s.chooseAgentForReplicaQuerier(ctx, tx, service.Spec, occupied); errors.Is(err, errNoPlacementAvailable) {
		...
		changed, markErr := markAllocationUnavailableForFailover(ctx, tx, allocation.ID, state, message, now)
		if markErr != nil {
	...
	if !ok || current.ResolvedSpec == nil || strings.TrimSpace(current.ImageDigest) == "" {
		message := "agent unhealthy; service has no reusable image snapshot for a rolling replacement"
		changed, markErr := markAllocationUnavailableForFailover(ctx, tx, allocation.ID, state, message, now)
		if markErr != nil {
			return result, markErr
		}
	...
	inProgress := hasRollout && (rollout.State == rolloutStateInProgress || rollout.State == rolloutStatePendingBuild)
	if inProgress && allocation.DesiredRolloutGeneration < service.RolloutGeneration {
		// Another generation is already rolling. Leave this predecessor in place
		// until that rollout can absorb it, matching drain's wait.
		return result, nil
	}
	...
	if _, err := s.copyDeploymentRolloutTx(ctx, tx, service, current, "", reasonFailoverRescheduled, detail); err != nil {
		return result, err
	}
	result.Replaced = true
	result.Changed = true
