...
func TestServiceLogsRuntimeIngestQueryAndIsolation(t *testing.T) {
	...
	if err := store.fleet.validateAgentLogBatch(ctx, agentID, spoofDrop); err == nil {
		t.Fatal("validateAgentLogBatch accepted a drop for another service")
	}
	ownedDrop := &agentv1.LogBatch{Drops: []*platformv1.LogDropSummary{{
		AllocationId: allocA, DroppedCount: 1,
	}}}
	if err := store.fleet.validateAgentLogBatch(ctx, agentID, ownedDrop); err != nil {
		t.Fatalf("validateAgentLogBatch rejected an owned drop: %v", err)
	}
