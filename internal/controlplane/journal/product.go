...
const serviceJSON = `jsonb_build_object(
	...
	'current_artifact_id', COALESCE((SELECT current_artifact_id FROM service_delivery_status WHERE service_id = services.id), ''),
	'current_resolved_image', COALESCE((SELECT a.image_ref FROM build_artifacts a JOIN service_delivery_status ds ON ds.current_artifact_id = a.id WHERE ds.service_id = services.id), ''),
...
const rolloutJSON = `jsonb_build_object('service_id', service_id, 'rollout_generation', rollout_generation, 'spec_revision', spec_revision, 'reason', reason, 'build_id', build_id, 'requested_by_user_id', requested_by_user_id, 'state', state, 'strategy_json', strategy_json, 'desired_replica_count', desired_replica_count, 'artifact_id', COALESCE(artifact_id, ''), 'image_digest', COALESCE((SELECT image_ref FROM build_artifacts WHERE id = service_rollouts.artifact_id), ''), 'failure_reason', failure_reason, 'target_allocation_id', target_allocation_id, 'completed_at', completed_at, 'progress_at', progress_at, 'created_at', created_at)`
const deploymentJSON = `jsonb_build_object('id', id, 'service_id', service_id, 'spec_revision', spec_revision, 'rollout_generation', rollout_generation, 'build_id', build_id, 'artifact_id', COALESCE(artifact_id, ''), 'image_digest', COALESCE((SELECT image_ref FROM build_artifacts WHERE id = deployments.artifact_id), ''), 'state', state, 'cause_kind', cause_kind, 'cause_id', cause_id, 'reason_code', reason_code, 'detail', detail, 'resolved_spec_json', resolved_spec_json, 'variable_versions_json', variable_versions_json, 'sealed_versions_json', sealed_versions_json, 'is_current', is_current, 'requested_by_user_id', requested_by_user_id, 'created_at', created_at, 'updated_at', updated_at)`
