...
func ControlPlane(args []string) (config.ControlPlaneConfig, error) {
	...
	intFlag(fs, &cfg.Logs.IngestQueueFlushes, "logs-ingest-queue-flushes", "CONTROLPLANE_LOGS_INGEST_QUEUE_FLUSHES", 512, "queued agent batches before whole batches shed with gap rows")
	intFlag(fs, &cfg.Logs.IngestQueueBytes, "logs-ingest-queue-bytes", "CONTROLPLANE_LOGS_INGEST_QUEUE_BYTES", 64<<20, "retained bytes of queued agent log batches before whole batches shed with gap rows")
	...
	intFlag(fs, &cfg.Builder.MaxConcurrentPerProject, "builder-max-concurrent-per-project", "CONTROLPLANE_BUILDER_MAX_CONCURRENT_PER_PROJECT", 5, "")
	intFlag(fs, &cfg.Builder.BuildTimeoutSeconds, "builder-timeout-seconds", "CONTROLPLANE_BUILDER_TIMEOUT_SECONDS", 1800, "")
	intFlag(fs, &cfg.Builder.MaxQueueAgeSeconds, "builder-max-queue-age-seconds", "CONTROLPLANE_BUILDER_MAX_QUEUE_AGE_SECONDS", 7200, "")
