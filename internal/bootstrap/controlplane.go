...
func ControlPlane(args []string) (config.ControlPlaneConfig, error) {
	...
	stringFlag(fs, &cfg.SecretKeys.Provider, "secret-keys-provider", "CONTROLPLANE_SECRET_KEYS_PROVIDER", "file", "file or aws-kms; production requires aws-kms")
	stringFlag(fs, &cfg.SecretKeys.File.Directory, "secret-keys-dir", "CONTROLPLANE_SECRET_KEYS_DIR", "", "development key file directory; defaults under the control-plane state directory")
	stringFlag(fs, &cfg.SecretKeys.KMS.Region, "secret-keys-kms-region", "CONTROLPLANE_SECRET_KEYS_KMS_REGION", "", "AWS region for the production KMS provider")
	stringFlag(fs, &cfg.SecretKeys.KMS.Endpoint, "secret-keys-kms-endpoint", "CONTROLPLANE_SECRET_KEYS_KMS_ENDPOINT", "", "optional KMS endpoint override for VPC endpoints or test doubles")
	stringFlag(fs, &cfg.SecretKeys.KMS.KeyID, "secret-keys-kms-key-id", "CONTROLPLANE_SECRET_KEYS_KMS_KEY_ID", "", "default KMS key for the installation's first envelope key")
	intFlag(fs, &cfg.SecretKeys.KMS.TimeoutSeconds, "secret-keys-kms-timeout-seconds", "CONTROLPLANE_SECRET_KEYS_KMS_TIMEOUT_SECONDS", 10, "")
	intFlag(fs, &cfg.SecretKeys.KMS.MaxAttempts, "secret-keys-kms-max-attempts", "CONTROLPLANE_SECRET_KEYS_KMS_MAX_ATTEMPTS", 5, "")
