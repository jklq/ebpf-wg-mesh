...
func Builder(args []string) (config.BuilderConfig, error) {
	...
	intFlag(fs, &cfg.Limits.TimeoutSeconds, "build-timeout-seconds", "BUILDER_BUILD_TIMEOUT_SECONDS", limits.TimeoutSeconds, "")
	int64Flag(fs, &cfg.Limits.MemoryBytes, "build-memory-bytes", "BUILDER_BUILD_MEMORY_BYTES", limits.MemoryBytes, "")
	int64Flag(fs, &cfg.Limits.CPUSeconds, "build-cpu-seconds", "BUILDER_BUILD_CPU_SECONDS", limits.CPUSeconds, "")
	int64Flag(fs, &cfg.Limits.MaxFileBytes, "build-max-file-bytes", "BUILDER_BUILD_MAX_FILE_BYTES", limits.MaxFileBytes, "")
	int64Flag(fs, &cfg.Limits.MaxProcesses, "build-max-processes", "BUILDER_BUILD_MAX_PROCESSES", limits.MaxProcesses, "")
	int64Flag(fs, &cfg.Limits.MaxWorkspaceBytes, "build-max-workspace-bytes", "BUILDER_BUILD_MAX_WORKSPACE_BYTES", limits.MaxWorkspaceBytes, "")
