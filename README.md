...
- WireGuard/eBPF is the private node-to-node transport/policy layer behind `internal/mesh`.
...
The binaries require no YAML config files.
...
- Agents do not need pre-generated client certificates. Each agent generates its own key in `AGENT_DATA_DIR/tls`, enrolls with `AGENT_BOOTSTRAP_TOKEN`, receives a short-lived mTLS certificate, and renews it automatically before expiry.
