...
PUBLIC_LISTEN=${PUBLIC_LISTEN:-0.0.0.0:8080}
...
[Service]
Type=simple
ExecStart=${CONTROLPLANE_BIN} -public-listen ${PUBLIC_LISTEN} -internal-listen ${INTERNAL_LISTEN} -agent-bootstrap-tokens ${BOOTSTRAP_TOKEN} -db-url postgresql://root@127.0.0.1:26257/defaultdb?sslmode=disable -state-dir ${STATE_DIR} -ingress-public-addr ${PUBLIC_ADDR} -bootstrap-user ${BOOTSTRAP_USER}
Restart=always
RestartSec=3
